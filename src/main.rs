...
fn main() {
    let nsname = if env::args().len() > 1 {
        env::args().nth(1).unwrap()
    } else {
        panic!("Please supply at least the network-namespace to this command.");
    };
    ...
    let args: Vec<_> = match env::args().len() {
        // only netns-name given.
        2 => vec![CString::new("bash").unwrap()],
        // more commands, skip bin-name and netns-name.
        _ => env::args()
                .into_iter()
                .skip(2)
